        ...
        div()
            ...
            .when_some(context_menu_pos, |this, pos| {
                ...
                let is_disk_image = selected_entry.as_ref()
                    ...
                    .unwrap_or(false);
                let is_iso_file = selected_entry.as_ref()
                    .map(|e| {
                        e.path.extension()
                            .and_then(|ext| ext.to_str())
                            .map(|ext| ext.to_lowercase() == "iso")
                            .unwrap_or(false)
                    })
                    .unwrap_or(false);
                    ...
                    anchored()
                        ...
                        .child(
                            div()
                                ...
                                .when(is_iso_file, |this| {
                                    let entity = entity.clone();
                                    let entry = selected_entry.clone();
                                    this.child(render_context_menu_item("usb", "Create Bootable USB", text_light, hover_bg, {
                                        move |_window, cx| {
                                            if let Some(ref e) = entry {
                                                entity.update(cx, |view, cx| {
                                                    view.pending_context_action = Some(ContextMenuAction::CreateBootableUSB(e.path.clone()));
                                                    view.close_context_menu();
                                                    cx.notify();
                                                });
                                            }
